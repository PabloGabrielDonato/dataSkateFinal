...
<head>
  ...
  <title>Rioplatense - Tabla Conbinadas</title>
</head>
<!-- Google tag (gtag.js) -->
...
<body class="">
        ...
        <table id="tablaClientes" class="display responsive nowrap compact table-bordered" style="width: 60%">
          <thead class="text-center">
            <tr>
              ...
              <th class="text-center">1° Programa</th>
              <th class="text-center">2° Programa</th>
      ...
      <div class="modal-content">
        ...
        <div class="col-lg-12">
          <div class="form-row">
            ...
            <div class="form-group col-md-4 form-floating">
              <select name="id_disciplina" class="form-control" id="consultarID_Disciplina" required>
                <option value=""></option>
                <?php //Completa el SELECT on los datos de la tabla
                $select = mysqli_query($con, "SELECT disciplinas.id, disciplinas.disciplina FROM disciplinas");
                while ($disciplina = mysqli_fetch_row($select)) {
                ?>
                  <option value="<?php echo ($disciplina[0]) ?>"><?php echo ($disciplina[1]) ?></option>
                <?php } ?>
              </select>
            ...
            <div class="form-group col-md-4 form-floating">
              <input name="programa1" type="text" class="form-control form-control-user" id="consultarPrograma1" placeholder="Primer programa" required />
              <label for="floatingInput">Primer programa</label>
            </div>

            <div class="form-group col-md-4 form-floating">
              <input name="programa2" type="text" class="form-control form-control-user" id="consultarPrograma2" placeholder="Segundo programa" required />
              <label for="floatingInput">Segundo programa</label>
            </div>
          </div>
  ...
  <!-- <form class="modal fade" id="formAlta" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" method="POST" action="api/combinada.php"> -->
  <form class="modal fade" id="formAlta" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title" id="exampleModalLabel">Alta Combinada</h4>
          <button class="close" type="button" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span>
          </button>
        </div>
        <div class="col-lg-12">
          ...
          <div class="form-row">
            ...
            <div class="form-group col-md-4 form-floating">
              <select name="id_disciplina" class="form-control" id="altaID_Disciplina" required>
                <option value=""></option>
                <?php //Completa el SELECT on los datos de la tabla
                $select = mysqli_query($con, "SELECT disciplinas.id, disciplinas.disciplina FROM disciplinas");
                while ($disciplina = mysqli_fetch_row($select)) {
            ...
            <div class="form-group col-md-4 form-floating">
              <input name="programa1" type="text" class="form-control form-control-user" id="altaPrograma1" placeholder="Primer programa" required />
              <label for="floatingInput">Primer programa</label>
            </div>

            <div class="form-group col-md-4 form-floating">
              <input name="programa2" type="text" class="form-control form-control-user" id="altaPrograma2" placeholder="Segundo programa" required />
              <label for="floatingInput">Segundo programa</label>
            </div>
  ...
  <script>
    ...
    function consultarUnCliente(id) {
      ...
      fetch(
          `api/combinada.php?id=${id}&opcion=${opcion}`, {
            method: 'GET',
            headers: {
        ...
        .then((datos) => {
          ...
          $("#consultarID_Disciplina").val(datos[0].id_disciplina);
          $("#consultarPrograma1").val(datos[0].programa1);
          $("#consultarPrograma2").val(datos[0].programa2);
    ...
    var urlEdit = 'api/combinada.php';
    // === FIN ACTUALIZAR DATOS ===

    // === ALTA de DATOS ===
    var urlAlta = 'api/combinada.php';
    // === FIN Alta DATOS ===
