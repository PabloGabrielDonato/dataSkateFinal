...
<head>
  ...
  <title>Rioplatense - Tabbla Relaciones</title>
</head>
<!-- Google tag (gtag.js) -->
...
<body class="">
  ...
  <div class="pcoded-main-container mt-4">
    <div class="text-center">
      <h1>Tabla Relaciones (Divional-Disciplina-Eficiencia-Categoria)</h1>
    </div>
    <div class="col-lg-12">
          ...
          <div class="form-row">
            ...
            <div class="form-group col-md-3 form-floating">
              <select name="categoria" class="form-control form-control-edit" id="consultarCategoria" required>
                ...
                while ($opcion = mysqli_fetch_row($select)) { ?>
                  <option value="<?php echo $opcion[0] ?>"><?php echo ($opcion[1] . ' - (' . $opcion[3] . '-' . $opcion[4] . ')') ?></option>
                <?php  }
                ?>
              </select>
  ...
  <!-- <form class="modal fade" id="formAlta" action="api/relaciones.php" method="POST"> -->
  <form class="modal fade" id="formAlta" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title" id="exampleModalLabel">Alta de Relaciones</h4>
          <button class="close" type="button" data-bs-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span>
          </button>
        </div>
        <div class="col-lg-12">
          ...
          <div class="form-row">
            <div class="form-group col-md-3 form-floating">
              <select name="divisional" class="form-control" id="altaDivisional" required>
                <option value=""></option>
                <!-- <option value="">Seleccionar Categoria</option> -->
                <?php
                $select = mysqli_query($con, "SELECT * FROM divisional");
  ...
  <script>
    ...
    function consultarUnCliente(id) {
      ...
      fetch(
          `../../api/relaciones.php?id=${id}&opcion=${opcion}`, {
            method: 'GET',
            headers: {
        ...
        .then((datos) => {
          ...
          $("#consultarDivisional").val(datos[0].id_divisional);
          $("#consultarDisciplina").val(datos[0].id_disciplina);
          $("#consultarEficiencia").val(datos[0].id_eficiencia);
          $("#consultarCategoria").val(datos[0].id_categoria);
    ...
    var urlEdit = '../../api/relaciones.php';
    // === FIN ACTUALIZAR DATOS ===

    // === ALTA de DATOS ===
    var urlAlta = '../../api/relaciones.php';
    // === FIN Alta DATOS ===
